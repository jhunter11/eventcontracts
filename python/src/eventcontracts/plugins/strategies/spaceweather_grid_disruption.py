"""Space-weather grid disruption strategy.

Thesis: geomagnetic storm markets and grid-disruption proxies can lag NOAA/SWPC
solar-wind measurements, CME ensemble arrival updates, and Kp nowcasts. This
strategy trades a disruption or Kp-threshold market from a calibrated
space-weather probability signal.
"""
...
class SpaceWeatherGridDisruptionStrategy(StrategyBase):
    """Slow CME/Kp probability edge on space-weather markets."""

    def __init__(self, spec: StrategySpec) -> None:
        super().__init__(spec)
        self.signal_source = str(spec.parameters.get("signal_source", "swpc-nowcast"))
        self.min_edge_bps = Decimal(str(spec.parameters.get("min_edge_bps", "120")))
        self.min_kp_nowcast = Decimal(str(spec.parameters.get("min_kp_nowcast", "5.0")))
    ...
    def on_event(self, event: NormalizedEvent, ctx: StrategyContext) -> Sequence[StrategyDecision]:
        ...
        if not isinstance(event, ExternalSignalEvent) or event.source != self.signal_source:
            return (NoAction(reason="ignored:not_spaceweather_signal"),)

        market_id = _market_id(event.payload)
        probability = _bounded_decimal(event.payload.get("disruption_probability"))
        kp = _decimal(event.payload.get("kp_nowcast")) or Decimal("0")
        if market_id is None or probability is None:
            return (NoAction(reason="censored:missing_market_or_probability"),)
        if kp < self.min_kp_nowcast:
            return (NoAction(reason="kp_filter_not_met"),)
        ...
        return (
            _place(
                ...
                edge_bps=edge_bps,
                size=self.size,
                reason=f"spaceweather kp={kp} prob={probability}",
            ),
        )
...
@register("spaceweather_grid_disruption")
def factory(spec: StrategySpec) -> SpaceWeatherGridDisruptionStrategy:
    return SpaceWeatherGridDisruptionStrategy(spec)
